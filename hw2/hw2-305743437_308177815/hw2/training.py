    ...
    def fit(
        ...
        **kw,
        ...
        for epoch in range(num_epochs):
            ...
            # Train
            actual_num_epochs += 1
            epoch_loss, epoch_acc = self.train_epoch(dl_train, **kw)
            train_loss.append(epoch_loss)
            train_acc.append(epoch_acc)
            # Test
            epoch_loss, epoch_acc = self.test_epoch(dl_test, **kw)
            test_loss.append(epoch_loss)
            test_acc.append(epoch_acc)
            
            if early_stopping and epoch > 1 and abs(test_acc[-1] - test_acc[-2]) < early_stopping:
                break
...
class LayerTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        # ====== YOUR CODE: ======
        # Forward pass
        y_pred = self.model(X.view(X.shape[0],-1))
        ...
        self.optimizer.step()
        # Calculate accuracy
        num_correct = torch.sum(y==y_pred.argmax(dim=1))
        # ========================
...
class TorchTrainer(Trainer):
    ...
    def train_batch(self, batch) -> BatchResult:
        ...
        _, pred = torch.max(y_pred, dim=1) # get the predicted index
