...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        positive_grad = 1
        negative_grad = self.alpha
        # gradient of the output dout w.r.t to the input x
        x_positive = torch.tensor(dout * positive_grad)
        x_negative = torch.tensor(dout * negative_grad)
        dx = torch.where(x > 0, x_positive, x_negative)
        # ========================
...
class Sigmoid(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        out = 1/(1+torch.exp(-x)) # sigmoid(x)
        self.out = out # save tanh(x) in self to use un bacward()
        # ========================
...
class TanH(Layer):
    ...
    def forward(self, x, **kw):
        ...
        #  Save whatever you need into grad_cache.
        # ====== YOUR CODE: ======
        out = (torch.exp(x) - torch.exp(-x)) / (torch.exp(x) + torch.exp(-x)) # tanh(x)
        self.out = out # save tanh(x) in self to use un bacward()
        # ========================
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        # gradient of the output dout w.r.t to the input x
        dx = (1 - torch.pow(self.out,2)) * dout
        # ========================
...
class Linear(Layer):
    ...
    def __init__(self, in_features, out_features, wstd=0.1):
        ...
        #tip: use torch.randn
        # ====== YOUR CODE: ======
        self.w = torch.normal(size=(out_features, in_features), mean=0, std=wstd)
...
class CrossEntropyLoss(Layer):
    ...
    def backward(self, dout=1.0):
        ...
        # ====== YOUR CODE: ======
        x_exp = torch.exp(x)
        exp_sum = x_exp.sum(dim=1)
        specific_ones = torch.zeros(x.shape)
        specific_ones.index_put_((torch.tensor(range(x.shape[0])), y), torch.tensor([1.]))

        dx = ((torch.div(x_exp.T, exp_sum).T - specific_ones) * dout) / N
        # ========================
...
class MLP(Layer):
    ...
    def __init__(
        ...
        **kw,
        ...
        # ====== YOUR CODE: ======
        for idx, h in enumerate(hidden_features):
            if idx == 0:
