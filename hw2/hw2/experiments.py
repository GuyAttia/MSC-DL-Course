...
def run_experiment(
    ...
    conv_params=dict(kernel_size=(3), stride=(1,1), padding=(1,1)),
    ...
    **kw,
    ...
    # ====== YOUR CODE: ======
    # Data - use DataLoader
    train_dl = torch.utils.data.DataLoader(ds_train, batch_size=bs_train)
    test_dl = torch.utils.data.DataLoader(ds_test, batch_size=bs_test)
    ...
    model = model_cls(in_size=in_size,
                     out_classes=10,
                     channels = [c for c in filters_per_layer for _i in range(layers_per_block)],
    ...
    fit_res = trainer.fit(train_dl, test_dl, epochs, max_batches=batches, print_every=4,early_stopping=early_stopping)
