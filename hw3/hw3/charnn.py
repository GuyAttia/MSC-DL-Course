...
def char_maps(text: str):
    ...
    # ====== YOUR CODE: ======
    uniqe_chars_in_text = list(set(text))
    uniqe_chars_in_text.sort()
    char_to_idx = {char:idx for idx, char in enumerate(uniqe_chars_in_text)}
...
def onehot_to_chars(embedded_text: Tensor, idx_to_char: dict) -> str:
    ...
    for row in embedded_text:
        index = torch.argmax(row, axis=0)
        result += idx_to_char[index.item()]
    # ========================
    return result
...
def generate_from_model(model, start_sequence, n_chars, char_maps, T):
    ...
    with torch.no_grad():
        ...
        start_sequence_onehot = start_sequence_onehot.unsqueeze(0).type(torch.float).to(device)
        
        y, h = model(start_sequence_onehot)
        y = y.squeeze(0)[-1, :]
        ...
        while len(out_text) < n_chars:
            y_onehot = chars_to_onehot(y, char_to_idx)
            y_onehot = y_onehot.unsqueeze(0).type(torch.float).to(device)
            
            y, h = model(y_onehot, h)
            y = y[0, -1, :]
...
class MultilayerGRU(nn.Module):
    ...
    def __init__(self, in_dim, h_dim, out_dim, n_layers, dropout=0):
        ...
        for i, layer in enumerate(range(n_layers)):
            self.layer_params.append(
                {'H_z': nn.Linear(h_dim, h_dim, bias=True),
                 'X_z': nn.Linear(in_dim, h_dim, bias=False),
                 'H_r': nn.Linear(h_dim, h_dim, bias=True),
                 'X_r': nn.Linear(in_dim, h_dim, bias=False),
                 'H_g': nn.Linear(h_dim, h_dim, bias=True),
                 'X_g': nn.Linear(in_dim, h_dim, bias=False)})
            
            in_dim = h_dim
            
            for linear_module in self.layer_params[-1]:
                self.add_module(f"{i}_{linear_module}", self.layer_params[-1][linear_module])
                
        self.layer_params.append(nn.Linear(h_dim, out_dim, bias=True))
        self.add_module("out_layer", self.layer_params[-1])
        # ========================
    ...
    def forward(self, input: Tensor, hidden_state: Tensor = None):
        ...
        for seq in range(seq_len):
            ...
            h_prev = torch.stack(layer_states, dim=1)
            inp = layer_input[:, seq, :]
            layer_states = []

            for k, layer in enumerate(self.layer_params[:-1]):
                H_k = h_prev[:, k, :]
                z = sigmoid(layer['X_z'](inp) + layer['H_z'](H_k))
                r = sigmoid(layer['X_r'](inp) + layer['H_r'](H_k))
                g = tanh(layer['X_g'](inp) + layer['H_g'](r * H_k))
                h = z * H_k + (1 - z) * g
                layer_states.append(h.clone())
                inp = dropout(h)
                
            layer_output.append(self.layer_params[-1](inp))   
